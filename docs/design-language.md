...
**Mechanism:** an element that owns a position sets `--item-hue` (`hueVars`) and carries `.glim-hue`; the stylesheet redefines `--accent` **for that subtree**. Every component that already paints activity picks the colour up automatically — none of them has to know the mode exists. `.glim-tint` washes the whole element at 16% (as an inset shadow, so the row's own hover tint still shows through); `.glim-tint-badge` is the same mechanism at a stronger 50%, for a compact circular badge rather than a list row (below); `.glim-tint-hover` is the same wash but rest-quiet, revealed only on hover/focus (below). `.glim-hue-icon` additionally colours a glyph — situational, see the icon-only badge bullet next, not a default pairing. The set's active/selected member additionally carries **`.glim-active`** (the shared marker class, not just whatever app-specific "this one is selected" class already exists alongside it) — `.glim-tint.glim-active` reads the stronger 22% soft tint instead of the plain wash, so the selected row still reads as distinct from its neighbours instead of collapsing into the same flat colour as everything else in the list.

**An icon-only badge that owns a position carries `.glim-hue` + `.glim-tint-badge` (or `.glim-tint-hover`) — the badge's own background, NOT the glyph.** An earlier revision of this rule additionally recoloured the glyph via `.glim-hue-icon`, reasoning that a badge with only its background washed and a flat-coloured glyph on top read as half-applied — a real adopting app tried exactly that and immediately rejected it (jdp: "die icons sollen nicht eingefärbt werden, nur die badges also der hintergrund"). The glyph stays `currentColor`/neutral; only the surface behind it takes the position. `.glim-hue` is still required unconditionally (not just while active/checked) for the ACTIVE/checked fill to resolve `--accent` correctly — `.glim-hue-icon` is a separate, situational tool for a genuinely backgroundless glyph (nothing behind it TO wash), not the default for a real badge. Use `.glim-tint-badge`, not the plain `.glim-tint`: the same adopting app that settled on background-only colouring came back once it shipped to report the badges still read as barely tinted (jdp: "die ganzen icon badges sind immer noch schwach eingefärbt, die sollen normal kräftig eingefärbt sein") — `.glim-tint`'s 16% is calibrated for a dense LIST (rule below: strong enough to register, weak enough not to overwhelm a whole column of rows), and a single small isolated badge with no neighbours to reinforce the colour needs markedly more of it to read as clearly coloured rather than barely-tinted grey. Contrast this with a **horizontal selector's own segments**, below: there the inactive members get NEITHER `.glim-hue` nor any tint at all — a selector's fill already says which one is picked, and washing the rest too is noise the selection state doesn't need, unlike a badge or a list where the idle state needs its own colour or it shows nothing (the trap above).
