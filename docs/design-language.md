...
- **Accent** overrides `--accent`; `--accent-contrast` is **computed** from sRGB luminance, never asked for as a second setting — asking for a second colour just to make the first one legible is a trap, not a preference. **The default, before anyone has touched the setting, is Sunflower gold `#FCC419`** (`DEFAULT_ACCENT` in `reference/appearance.ts`) — a fresh install of any app that shares this language opens in the same colour, so the family reads as one product from the very first screen, not just after someone finds the picker.
  - **The picker is one row: preset swatches, the custom colour input, and a reset badge, all the SAME size.** A native `<input type="color">` sized differently from the swatches beside it (a common default: the browser's own colour input renders as a squat rectangle, not a circle/pill matching the presets) reads as a different control glued on afterward rather than one more option in the same set — give it the identical swatch class the presets use, sized and shaped identically (including following the shape engine, below). The reset control is a **badge with an icon** (a counter-clockwise arrow), not a text link — see rule 13 — pinned to the row's trailing edge with `margin-inline-start: auto`, not left inline with the swatches, so it doesn't visually compete with the presets it's resetting away from.
  - **Swatches follow the shape engine.** `border-radius: var(--radius-pill)` on each preset (and the custom-input/reset badge sharing that class) — Round keeps them circular, Soft/Square progressively square them off, the same way any other shape-reactive control does. This isn't automatic just from adopting the shared token: an app has to actually point its own swatch CSS at `--radius-pill` rather than a hard-coded `50%`.
