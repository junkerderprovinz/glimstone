...
- **Accent** overrides `--accent`; `--accent-contrast` is **computed** from sRGB luminance, never asked for as a second setting — asking for a second colour just to make the first one legible is a trap, not a preference. **The default, before anyone has touched the setting, is Sunflower gold `#FCC419`** (`DEFAULT_ACCENT` in `reference/appearance.ts`) — a fresh install of any app that shares this language opens in the same colour, so the family reads as one product from the very first screen, not just after someone finds the picker.
  - **The custom colour control is a permanently embedded picker, never a native `<input type="color">`.** A native colour input hands off to a browser/OS surface entirely outside the page — on many setups a genuinely separate top-level window, not an inline popover — which is exactly what this rule exists to rule out (jdp, building CannonadeCommand: *"ich will das Farbwählfeld fest integriert"* — I want the colour field permanently embedded; the same rejection recurred nearly verbatim in an adopting app that tried the native-input shortcut anyway: *"es soll sich kein komplett neues Fenster öffnen"*). It's also functionally unverifiable in automated testing — a native picker's surface sits outside the page's own DOM, so nothing can prove it opens, let alone opens correctly. Use `reference/colorPicker.ts`'s `colorPicker()` instead: a real saturation/value square + hue bar (`.glim-picker`/`.glim-picker-sv`/`.glim-picker-hue`), drawn in the app's own DOM, always visible — CannonadeCommand's own `inlinePicker()`, the pattern this was extracted from.
  - **Layout: swatches first, the embedded picker revealed on demand for a multi-value palette; permanently visible for a single value.** A single custom colour (the app's own accent) gets the picker shown at all times, sitting above the row of preset swatches — clicking a preset both applies it AND re-syncs the picker's own dot positions to match (`setValue()`), so the picker always reflects the current colour whichever way it was set. A *multi-value* palette (rainbow's 8 positions) shares ONE picker instance across all of them: hidden until a swatch is first clicked, then revealed directly below the swatch row and re-synced to edit whichever swatch was clicked most recently — never one picker per swatch (eight always-open 2D controls stacked in a settings page is more chrome than any of them individually earns).
  - **Preset/palette swatches stay flat colour circles, not pickers of their own.** Each is a plain filled circle (`background-color` set from its own hex, `border-radius: var(--radius-pill)`, a border-colour highlight on whichever is active — rule: "Voreingestellten Farben gekennzeichnet wie in BombVault") that a click selects; only the ONE shared/embedded picker element does any dragging. A reset control is a **badge with an icon** (a counter-clockwise arrow), not a text link — see rule 13 — pinned to the row's trailing edge with `margin-inline-start: auto`.
...
9. For any custom-colour control (accent, a rainbow palette position), copy [`reference/colorPicker.ts`](../reference/colorPicker.ts) and call `colorPicker(hex, onChange)` instead of reaching for a native `<input type="color">` — see the picker bullets under "The user-owned axes" above for the always-visible-vs-revealed-on-click layout rule.
