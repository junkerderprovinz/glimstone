...
- **Wiring ONE set is not the same as wiring the mode.** An adopting app that hooks rainbow up to only its most obvious list (a download queue, say) has implemented a feature that colours one part of the screen, not "rainbow mode" — and a user comparing it against a sibling app where the whole surface goes rainbow-lit (badges, segmented-control segments, icon toggles, every equal-member set) will keep reporting "it doesn't apply" no matter how many times the one wired set is fixed, because their own bar for "applied" was never that one list. `rainbowColor(i)`/`hueVars()` are already general-purpose — nothing about them is queue-specific — so the fix is coverage, not new mechanism: walk every genuine equal-member set in the app (rule: "anything that is the only one of its kind on the page keeps the single accent" is the ONLY exclusion) and wire each one, not just the first one that looked like an obvious fit. `.glim-hue-icon` also needs `.glim-hue` sitting on the SAME element, not just a checked/active state — the icon-recolour rule only ever reads an `--accent` that something has already redefined, so a standalone icon-only toggle (not nested inside a row that already carries `.glim-hue`) has to carry both classes itself, unconditionally, or its glyph silently falls back to the page's single global accent instead of its own position.
